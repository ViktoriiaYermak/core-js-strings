...
function countVowels(str) {
  const vowelSet = ['a', 'e', 'i', 'o', 'u', 'A', 'E', 'I', 'O', 'U', 'y', 'Y'];
  let result = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (vowelSet.includes(str[i])) {
      result += 1;
    }
  }
  return result;
...
function isPalindrome(str) {
  const toLowerCaseStr = str.toLowerCase().replaceAll(/[^a-zA-Z0-9]/g, '');
  const reversStr = toLowerCaseStr.split('').reverse().join('');
  if (toLowerCaseStr === reversStr) {
    return true;
  }
  return false;
}
